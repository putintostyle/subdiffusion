...
%                 root = M\b;
            elseif iteration == 2
                ...
                b = (1-5/12)*source-(q1(1)+q(1)/2).*history_arr(:,end)+(q1(2)-q2(1)+(q(2)-q(1))/2).*(history_arr(:,end)-history_arr(:,end-1));
                ...
                b = source+sum(diff_b(:,1:end).*q1_r, 2)-sum(diff_b(:, 1:end).*q2_r, 2)-(q1(1)+q(1)/2).*history_arr(:,end)+sum(diff_b(:, 1:end).*q_r_1, 2)/2-sum(diff_b(:, 1:end).*q_r, 2)/2;
                root  = M\b;
